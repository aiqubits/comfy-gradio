...
def create_demo(model_path, save_path):
    ...
    with gr.Blocks(theme=gr.themes.Soft()) as demo:
        gr.Markdown("# Office Intelligence Video Generation")
        
        with gr.Row():
            with gr.Column():
                prompt = gr.Textbox(label="提示词", value="A cat walks on the grass, realistic style.")
                with gr.Row():
                ...
                num_inference_steps = gr.Slider(1, 100, value=50, step=1, label="推理步数")
                show_advanced = gr.Checkbox(label="显示高级选项", value=False)
...
if __name__ == "__main__":
    ...
    demo = create_demo("", "")
    demo.launch(server_name=server_name, server_port=server_port, share=True, debug=True, auth_message="Office Intelligence 系统", auth=("123", "321"))

        # demo.launch(server_name=server_name, server_port=server_port, share=True, debug=True, auth=["123", "456"] auth_message="None")